  ...
  <form action="/dashboard/transactions" method="post">
    ...
    <div>
      <div class="relative mb-3" data-te-inline="true" data-te-datepicker-init data-te-input-wrapper-init>
        <input type="text" id="date" name="date" class="peer block min-h-[auto] w-full rounded-lg border-0 bg-white p-2.5 outline-none transition-all duration-200 ease-linear focus:placeholder:opacity-100 peer-focus:text-primary data-[te-input-state-active]:placeholder:opacity-100 motion-reduce:transition-none [&:not([data-te-input-placeholder-active])]:placeholder:opacity-0" required placeholder="Select a date" data-te-datepicker-toggle-ref data-te-datepicker-toggle-button-ref />
      ...
      <div class="my-2">
        <input type="number" name="amount" id="amount" class="@error('amount') is-invalid @enderror border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-2.5 outline-none" placeholder="Amount" required autocomplete="off" value="{{ old('amount') }}">
        @error('amount')
      ...
      <div class="my-2">
        <input type="text" name="title" id="title" class="@error('title') is-invalid @enderror border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-2.5 outline-none" placeholder="Title" required autocomplete="off" value="{{ old('title') }}">
        @error('title')
          <div class="text-red-400">
            {{ $message }}
          </div>
      ...
      <div class="my-2">
        <input type="text" name="category" id="category" class="@error('category') is-invalid @enderror border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-2.5 outline-none" placeholder="Category" required autocomplete="off" value="{{ old('category') }}">
        @error('category')
          <div class="text-red-400">
            {{ $message }}
          </div>
      ...
      <div class="my-2">
        <input type="text" name="desc" id="desc" class="@error('desc') is-invalid @enderror border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-2.5 outline-none" placeholder="Description (optional)" autocomplete="off" value="{{ old('desc') }}">
        @error('desc')
          <div class="text-red-400">
            {{ $message }}
          </div>
      ...
      <div class="flex gap-4">
        <button type="button" onclick="location.href='/dashboard/transactions'" class="group relative p-2 w-full overflow-hidden rounded-lg bg-green-900 text-lg mt-4 border border-gray-500">
