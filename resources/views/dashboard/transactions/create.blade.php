  ...
  <form action="/dashboard/transactions" method="post">
    ...
    <div>
      <div class="relative mb-3 md:w-64 lg:w-96" data-te-inline="true" data-te-datepicker-init data-te-input-wrapper-init>
        <input type="text" id="date" name="date" class="peer block min-h-[auto] w-full rounded-lg bg-accent text-white p-2.5 border-0 outline-none transition-all duration-200 ease-linear focus:placeholder:opacity-100 peer-focus:text-blue-400 data-[te-input-state-active]:placeholder:opacity-100 motion-reduce:transition-none [&:not([data-te-input-placeholder-active])]:placeholder:opacity-0" required placeholder="Select a date" data-te-datepicker-toggle-ref data-te-datepicker-toggle-button-ref value="{{ old('date') }}" />
      ...
      <div class="my-2 md:w-96 lg:w-9/12">
        <input type="number" name="amount" id="amount" class="@error('amount') is-invalid @enderror bg-accent text-white text-sm rounded-lg w-full p-2.5 outline-none" placeholder="Amount" required autocomplete="off" value="{{ old('amount') }}">
        @error('amount')
      ...
      <div class="my-2 md:w-96 lg:w-9/12">
        <input type="text" name="title" id="title" class="@error('title') is-invalid @enderror bg-accent text-white text-sm rounded-lg w-full p-2.5 outline-none" placeholder="Title" required autocomplete="off" value="{{ old('title') }}">
        @error('title')
          <div class="text-red-400">
            {{ $message }}
          </div>
      ...
      <div class="my-2 md:w-96 lg:w-9/12">
        <input type="text" name="category" id="category" class="@error('category') is-invalid @enderror bg-accent text-white text-sm rounded-lg w-full p-2.5 outline-none" placeholder="Category" required autocomplete="off" value="{{ old('category') }}">
        @error('category')
          <div class="text-red-400">
            {{ $message }}
          </div>
      ...
      <div class="my-2">
        <input type="text" name="desc" id="desc" class="@error('desc') is-invalid @enderror bg-accent text-white text-sm rounded-lg w-full p-2.5 outline-none" placeholder="Description (optional)" autocomplete="off" value="{{ old('desc') }}">
        @error('desc')
          <div class="text-red-400">
            {{ $message }}
          </div>
