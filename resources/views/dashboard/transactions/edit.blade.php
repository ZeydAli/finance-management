...
@section('container')
  <form action="/dashboard/transactions/{{ $transaction->id }}" method="post">
    ...
    <div class="flex flex-col items-center justify-center">
      <div class="relative mb-3 w-10/12 md:w-8/12 lg:w-6/12 xl:w-4/12" data-te-inline="true" data-te-datepicker-init data-te-input-wrapper-init data-te-format="yyyy-mm-dd">
        <input type="text" id="date" name="date" class="peer block min-h-[auto] w-full rounded-lg bg-component text-white p-2.5 border-0 outline-none transition-all duration-200 ease-linear focus:placeholder:opacity-100 peer-focus:text-blue-400 data-[te-input-state-active]:placeholder:opacity-100 motion-reduce:transition-none [&:not([data-te-input-placeholder-active])]:placeholder:opacity-0" required placeholder="Select a date" data-te-datepicker-toggle-ref data-te-datepicker-toggle-button-ref value="{{ old('date', $transaction->date) }}" />
      ...
      <div class="my-2 w-10/12 md:w-8/12 lg:w-6/12 xl:w-4/12">
        ...
        <input type="number" name="amount" id="amount" class="@error('amount') is-invalid @enderror bg-component text-white  text-sm rounded-lg w-full p-2.5 outline-none" placeholder="Amount" required autocomplete="off" value="{{ old('amount',$transaction->amount) }}">
        @error('amount')
      ...
      <div class="my-2 w-10/12 md:w-8/12 lg:w-6/12 xl:w-4/12">
        ...
        <input type="text" name="title" id="title" class="@error('title') is-invalid @enderror bg-component text-white  text-sm rounded-lg w-full p-2.5 outline-none" placeholder="Title" required autocomplete="off" value="{{ old('title', $transaction->title ) }}">
        @error('title')
          <div class="text-red-400">
            {{ $message }}
          </div>
      ...
      <div class="my-2 w-10/12 md:w-8/12 lg:w-6/12 xl:w-4/12">
        ...
        <input type="text" name="category" id="category" class="@error('category') is-invalid @enderror bg-component text-white  text-sm rounded-lg w-full p-2.5 outline-none" placeholder="Category" required autocomplete="off" value="{{ old('category', $transaction->category ) }}">
        @error('category')
          <div class="text-red-400">
            {{ $message }}
          </div>
      ...
      <div class="my-2 w-10/12 md:w-8/12 lg:w-6/12 xl:w-4/12">
        ...
        <textarea name="desc" id="desc" rows="4" class="@error('desc') is-invalid @enderror block p-2.5 w-full text-sm rounded-lg bg-component border-gray-600 placeholder-gray-500 text-white" placeholder="Enter description... (optional)">{{ old('desc', $transaction->desc) }}</textarea>
        @error('desc')
          <div class="text-red-400">
            {{ $message }}
          </div>
      ...
      <div class="flex gap-4 justify-between mt-4 w-10/12 md:w-8/12 lg:w-6/12 xl:w-4/12">
        <button type="button" onclick="location.href='/dashboard/transactions?date={{ $transaction->date }}'" class="text-white flex items-center gap-2 font-semibold">
  ...
  <div class="flex flex-col items-center justify-center">
    <div class="mt-4 flex items-center justify-center w-10/12 md:w-8/12 lg:w-6/12 xl:w-4/12">
      <form action="/dashboard/transactions/{{ $transaction->id }}" method="post">
        @method('delete')
        @csrf
